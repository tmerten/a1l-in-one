"""Tests for work_items and commits aggregation queries."""
...
from project_health.providers.protocol import RawPREvent, RawIssueEvent, RawCommitEvent
...
@pytest.mark.asyncio
async def test_work_items_returns_prs_and_issues(db_session: AsyncSession, minimal_config):
...
@pytest.mark.asyncio
async def test_work_items_filters_by_datasource(db_session: AsyncSession, minimal_config):
    ...
    await writer.write_pull_requests("github", [
        RawPREvent(
            ...
            actor="alice",
            project="owner/repo",
            data={"title": "PR", "merged_at": now.isoformat(), "html_url": "url"},
        )
    ])
...
@pytest.mark.asyncio
async def test_work_items_active_returns_open_prs(db_session: AsyncSession, minimal_config):
...
@pytest.mark.asyncio
async def test_work_items_pagination(db_session: AsyncSession, minimal_config):
...
@pytest.mark.asyncio
async def test_commits_returns_sha_and_message(db_session: AsyncSession, minimal_config):
    ...
    assert items[0].url == "https://github.com/owner/repo/commit/abc123def456"
